...
import { saveExtractedItems, deleteItemFromCloud, saveGlobalConfig, COLLECTIONS } from '../../services/persistenceService';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ appData, onExit }) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!apiOk && selectedCategory !== 'CAMPUS_MAP') {
      alert("CRITICAL: API Key is missing. Check Environment Variables!");
      return;
    }
    ...
    reader.onload = async (event) => {
      ...
      try {
        if (selectedCategory === 'CAMPUS_MAP' && file.type.startsWith('image/')) {
          ...
          await saveGlobalConfig({ campusMapImage: base64 });
          setStatusMsg('Map Deployed Successfully! 🗺️');
        ...
        setStatusMsg('Gemini Extracting & Saving...');
        // 1. Extract & Classify via AI
        const extracted = await extractCategoryData(selectedCategory, content, mimeType);
        
        if (extracted && extracted.length > 0) {
          // 2. Persist directly to Firestore collection
          await saveExtractedItems(selectedCategory, extracted);
          setStatusMsg('Cloud Storage Verified! 🚀');
        } else {
          setStatusMsg('Extraction returned empty data.');
        }
      } catch (err) {
        console.error("Admin Persistence Error:", err);
  ...
  const handleDeleteItem = async (id: string) => {
    ...
    try {
      await deleteItemFromCloud(selectedCategory, id);
    } catch (err) {
      console.error("Cloud Deletion Error:", err);
      alert("Failed to delete item from Firestore.");
    }
  };
      ...
      <header className="p-8 border-b border-slate-900 sticky top-0 bg-slate-950/80 backdrop-blur-xl z-20">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-black tracking-tighter text-blue-500">CLOUD ADMIN</h1>
          </div>
        ...
        <div className={`mt-4 py-1.5 px-4 rounded-full text-[8px] font-black uppercase tracking-widest text-center ${apiOk ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500 animate-pulse'}`}>
          {apiOk ? "● Direct Firestore Link Secure" : "● Connection Error (API Key Check)"}
        </div>
      </header>
        ...
        {selectedCategory ? (
          <div className="space-y-6 animate-slideUp">
            <div className="flex items-center justify-between">
              ...
              <h3 className="text-lg font-black text-white uppercase tracking-tighter">{CATEGORY_MAP[selectedCategory].label} Store</h3>
            ...
            <div className="bg-slate-900 border-2 border-slate-800 border-dashed rounded-[3rem] p-8 text-center group hover:border-blue-500/50 transition-all cursor-pointer"
              ...
              <h4 className="text-[11px] font-black text-slate-200 uppercase tracking-widest">Inject to Cloud</h4>
              <p className="text-[8px] text-slate-600 font-black mt-2 uppercase tracking-wide">Extracted data will be stored in Firestore collections</p>
            </div>
              ...
              {(appData[CATEGORY_MAP[selectedCategory].dataKey] as any[] || []).map((item: any) => (
                <div key={item.id} className="bg-slate-900 p-5 rounded-[2rem] border border-slate-800 flex justify-between items-center animate-fadeIn">
                  <div className="overflow-hidden">
                    <h5 className="text-[10px] font-black text-slate-200 truncate uppercase">{item.subject || item.name || item.title || item.role || item.day || "Cloud Record"}</h5>
                    <p className="text-[7px] text-slate-500 font-bold uppercase mt-1">ID: {item.id?.substring(0,8)}... • {item.branch || item.category || 'Global'}</p>
                  </div>
                  <button onClick={() => handleDeleteItem(item.id)} className="w-9 h-9 rounded-xl bg-slate-800 text-rose-500 hover:bg-rose-600 hover:text-white transition-all">
                    <i className="fa-solid fa-trash text-[11px]"></i>
                  </button>
                </div>
              ))}
              {(appData[CATEGORY_MAP[selectedCategory].dataKey] as any[] || []).length === 0 && (
                <div className="text-center py-10 opacity-30 italic text-[10px] uppercase tracking-widest font-black">Collection Empty</div>
              )}
            </div>
          </div>
        ) : (
          <div className="space-y-8">
            <div className="bg-gradient-to-br from-indigo-600 to-blue-800 p-10 rounded-[4rem] relative overflow-hidden group">
              <div className="relative z-10">
                <p className="text-blue-200 text-[9px] font-black uppercase tracking-widest mb-2">System Authority</p>
                <h2 className="text-3xl font-black text-white leading-none tracking-tighter mb-4 uppercase">Firestore<br/>Console</h2>
              ...
              <i className="fa-solid fa-server absolute -right-4 -bottom-4 text-9xl text-white/5 -rotate-12 group-hover:scale-125 transition-transform"></i>
            </div>
